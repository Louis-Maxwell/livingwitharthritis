...
|---|---|
| Copy tweak, one JSON/meta field, a short FAQ | cheaper / faster Claude (Sonnet low) |
| One named page or one bug with a known file | Sonnet medium |
| Several pages, SEO/AEO/GEO, prerender HTML | Sonnet high |
| Architecture, security, unknown root cause | Opus high |
...
- Hosting is Lovable-first today. `public/_redirects` and `public/_headers` are
  the Netlify-format files; there is no live Netlify URL until a human connects
  the repo. Do not invent a Netlify link.
