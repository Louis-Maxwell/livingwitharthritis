          ...
          {rest.map((a) => (
            <article key={a.slug} className="card-accent-top group bg-card rounded-2xl shadow-sm ring-1 ring-border overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all">
              <Link to={`/blog/${a.slug}`} className="block aspect-[16/10] overflow-hidden" tabIndex={-1} aria-label={a.title}>
                <img src={pickImage(a)} srcSet={imgSrcSet(pickImage(a))} sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" alt="" aria-hidden="true" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" decoding="async" />
              </Link>
              <div className="p-5">
