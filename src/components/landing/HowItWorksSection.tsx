    ...
    <div className="container mx-auto px-6 md:px-10 max-w-6xl">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {steps.map((step, i) => {
          ...
          return (
            <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.5 }} className="text-center relative">
              <div className="w-16 h-16 rounded-2xl bg-primary/6 flex items-center justify-center mx-auto mb-6 relative">
              ...
              {i < steps.length - 1 && <div className="hidden lg:block absolute top-8 left-[calc(100%_-_16px)] w-8 border-t-2 border-dashed border-border" />}
            </motion.div>
          );
        })}
      </div>
