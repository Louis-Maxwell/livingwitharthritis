...
function AnimatedStat({ stat }: { stat: typeof stats[0] }) {
  ...
  return (
    <div ref={ref} className="text-center group">
      ...
      <div className="w-8 h-[2px] bg-primary-foreground/20 mx-auto mb-3 group-hover:w-12 transition-all duration-300" />
      <p className="text-xs sm:text-sm text-primary-foreground/60 font-medium tracking-wide">{stat.label}</p>
    </div>
  );
}
...
const ImpactBannerSection = memo(() => (
  <section className="py-16 lg:py-20 bg-gradient-to-br from-primary via-primary to-primary/80 text-primary-foreground relative overflow-hidden">
    ...
    {/* Glow orbs */}
    <div className="absolute top-[-200px] right-[-100px] w-[500px] h-[500px] rounded-full bg-primary/15 blur-[140px] pointer-events-none" />
    <div className="absolute bottom-[-200px] left-[-100px] w-[400px] h-[400px] rounded-full bg-secondary/10 blur-[120px] pointer-events-none" />
