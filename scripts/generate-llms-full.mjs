...
async function main() {
  ...
  const allSections = [...headDataSections, ...blogSections];
  const output = [header, "", allSections.join("\n\n"), ""].join("\n");

  writeFileSync(resolve("public/llms-full.txt"), output);
  console.log(
    `[llms-full] wrote ${headDataSections.length} condition/guide sections + ${blogSections.length} blog sections (${allSections.length} total) -> public/llms-full.txt`,
  );
}
