...
The homepage now shows a United Kingdom coverage strip (England, Scotland,
Wales, Northern Ireland), a PIP/benefits start-here card, FAQ answers that
link through to the matching guide, and the same homepage title Google already
sees in the static page. Answer-engine text for the homepage is now shown on
the page, not only in the crawler file.
