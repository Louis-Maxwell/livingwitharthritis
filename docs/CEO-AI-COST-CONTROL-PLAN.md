...
- **Spend rules** restored on the evening visibility branch so later work stays
  on Cursor / Claude, not Lovable chat.
...
- The evening pass already tightened high-intent pages (pain relief, newly
  diagnosed, PIP, Mediterranean diet, turmeric) and crawl files (`llms.txt`,
  sitemap, IndexNow). This run keeps those and adds the homepage locks above.
